...
#ifdef SDT
XKrnl_vadd_Config *XKrnl_vadd_LookupConfig(UINTPTR BaseAddress) {
	XKrnl_vadd_Config *ConfigPtr = NULL;

	int Index;

	for (Index = (u32)0x0; XKrnl_vadd_ConfigTable[Index].Name != NULL; Index++) {
		if (!BaseAddress || XKrnl_vadd_ConfigTable[Index].Control_BaseAddress == BaseAddress) {
...
int XKrnl_vadd_Initialize(XKrnl_vadd *InstancePtr, UINTPTR BaseAddress) {
	XKrnl_vadd_Config *ConfigPtr;

	Xil_AssertNonvoid(InstancePtr != NULL);

	ConfigPtr = XKrnl_vadd_LookupConfig(BaseAddress);
	if (ConfigPtr == NULL) {
