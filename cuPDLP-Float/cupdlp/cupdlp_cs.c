...
cupdlp_float cupdlp_dcs_cumsum(int *p, int *c, int n) {
  int i, nz = 0;
  cupdlp_float nz2 = 0;
  ...
  for (i = 0; i < n; i++) {
    p[i] = nz;
    nz += c[i];
    nz2 += c[i]; /* also in cupdlp_float to avoid int overflow */
...
int cupdlp_dcs_entry(cupdlp_dcs *T, int i, int j, cupdlp_float x) {
