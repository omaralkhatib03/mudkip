...
void convert_double_to_float(const double *src, cupdlp_float *dest, size_t size) {
    for (size_t i = 0; i < size; i++) {
        dest[i] = (cupdlp_float)src[i];  // Explicit conversion
    }
}
...
cupdlp_retcode main(int argc, char **argv) {
  ...
  double sense_d = sense;
  double offset_d = offset;

  CUPDLP_CALL(formulateLP_highs(model2solve, &cost_d, &nCols_pdlp, &nRows_pdlp,
                                &nnz_pdlp, &nEqs_pdlp, &csc_beg, &csc_idx,
                                &csc_val_d, &rhs_d, &lower_d, &upper_d, &offset_d, &sense_d,
                                &nCols, &constraint_new_idx, &constraint_type));
  #ifdef SFLOAT
    sense = sense_d;
    offset = offset_d;
    cost  = (cupdlp_float *) malloc(nCols_pdlp * sizeof(cupdlp_float));
    lower = (cupdlp_float *) malloc(nCols_pdlp * sizeof(cupdlp_float));
    upper = (cupdlp_float *) malloc(nCols_pdlp * sizeof(cupdlp_float));
    rhs   = (cupdlp_float *) malloc(nRows_pdlp * sizeof(cupdlp_float));
    csc_val = (cupdlp_float*) malloc(nnz_pdlp * sizeof(cupdlp_float));
    
    convert_double_to_float(cost_d, cost, nCols_pdlp);
    convert_double_to_float(lower_d, lower, nCols_pdlp);
    convert_double_to_float(upper_d, upper, nCols_pdlp);
    convert_double_to_float(rhs_d, rhs, nRows_pdlp);
    convert_double_to_float(csc_val_d, csc_val, nnz_pdlp);
  ...
  memcpy(csc_cpu->colMatElem, csc_val, nnz_pdlp * sizeof(cupdlp_float));
#if !(CUPDLP_CPU)
...
exit_cleanup:
  ...
  /*if (cost_d != NULL) cupdlp_free(cost_d);*/
  /*if (rhs_d != NULL) cupdlp_free(rhs_d);*/
  /*if (lower_d != NULL) cupdlp_free(lower_d);*/
  /*if (upper_d != NULL) cupdlp_free(upper_d);*/
